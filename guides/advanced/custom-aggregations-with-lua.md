...
enough for your needs. In such cases the [`lua`][docs.reference.transforms.lua] transform comes to rescue, letting you
...
counts them, and emits [`counter`][docs.about.data-model.metric#counter] metrics each 5 seconds.
...
components. Read more about event representation [in the reference][docs.reference.transforms.lua#representation-of-events].
...
a dedicated function. Such a function can be placed into the [source][docs.reference.transforms.lua#source]
...
providing a [rich set][docs.reference.transforms] of predefined transforms for building production-grade observability
